...
from gs_renderer_fs import Renderer
...
class GUI:
    ...
    def prepare_train(self):
        ...
        if self.input_img is not None:
            ...
            if len(nonzero_idxes) > 0:
                ...
                # print(self.input_img_torch.shape, self.input_mask_torch.shape, self.input_depth_torch.shape, self.input_depth_mask_torch.shape)
                # torch.Size([1, 3, 256, 256]) torch.Size([1, 1, 256, 256]) torch.Size([1, 1, 256, 256]) torch.Size([1, 1, 256, 256])
                if self.opt.feature_splatting:
                    print('using dino feature splatting')
                    dino = load_dino(self.input_img_torch.device)
                    img_for_dino = self.input_img_torch            # [1,3,256,256]
                    pix_feats = dino_per_pixel_features(img_for_dino, dino)
                    print('dino feature shape', pix_feats.shape)  # [1,384,256,256]
                    self.input_feat_torch = pix_feats
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            # print(image.shape, 'known view image shape')
            ...
            if self.opt.feature_splatting:
                feat = out["extra"].unsqueeze(0)
                # print(feat.shape, 'known view feature shape')
                # feat = torch.nan_to_num(feat)
                loss = loss + 1000 * step_ratio * F.mse_loss(feat, self.input_feat_torch)
                print(f'feature loss: {1000 * F.mse_loss(feat, self.input_feat_torch).item()}')
    ...
    def train(self, iters):
        ...
        # Save
        self.renderer.save_global_motion(os.path.join(self.opt.outdir, "gaussians", str(self.opt.save_path)+"_global_motion.pkl"))
