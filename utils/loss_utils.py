...
def local_pearson_loss(depth_src, depth_target, depth_valid, box_p, p_corr):
    ...
    for i in range(len(x_0)):
        depth_src_box = depth_src[x_0[i]:x_1[i],y_0[i]:y_1[i]].reshape(-1)
        depth_target_box = depth_target[x_0[i]:x_1[i],y_0[i]:y_1[i]].reshape(-1)
        depth_valid_box = depth_valid[x_0[i]:x_1[i],y_0[i]:y_1[i]].reshape(-1) > 0.5
        if depth_valid_box.sum() > 0.1 * torch.ones_like(depth_valid_box).sum():
            _loss += pearson_depth_loss(depth_src_box[depth_valid_box], depth_target_box[depth_valid_box])
    return _loss/n_corr
