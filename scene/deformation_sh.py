...
class Deformation(nn.Module):
    def __init__(self, D=8, W=256, input_ch=27, input_ch_time=9, skips=[], args=None, use_res=False):
        ...
        self.num_bases = 10 # 20
        self.motion_bases_mlp = nn.Sequential(
            nn.Linear(1, self.W),
            nn.ReLU(),
            nn.Linear(self.W, self.num_bases * 3)  # Each motion basis is a 3D vector
        )
        ...
        if not self.use_res:
            self.coeff_predictor, self.sh_deform, self.scales_deform, self.rotations_deform, self.opacity_deform = self.create_net()
        else:
            self.coeff_predictor, self.sh_deform, self.scales_deform, self.rotations_deform, self.opacity_deform = self.create_res_net()
        self.args = args
        self.fix_time_embed = None
        
        # self.coeff_predictor = nn.Linear(self.W, self.num_bases)
        self.initialize_weights()
    
    def initialize_weights(self,):
        for m in self.motion_bases_mlp.modules():
            if isinstance(m, nn.Linear):
                init.constant_(m.weight, 0)
                # init.xavier_uniform_(m.weight,gain=1)
                if m.bias is not None:
                    # init.xavier_uniform_(m.bias,gain=1)
                    init.constant_(m.bias, 0)
        for m in self.sh_deform.modules():
            if isinstance(m, nn.Linear):
                init.constant_(m.weight, 0)
                # init.xavier_uniform_(m.weight,gain=1)
                if m.bias is not None:
    ...
    def create_res_net(self,):
        ...
        output_dim = self.W
        return  \
            Head_Res_Net(self.W, self.num_bases), \
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb, time_emb):
        ...
        # if self.fix_time_embed is None:
        #     self.fix_time_embed = time_emb
        # print('time embed:', self.fix_time_embed)
        # motion_bases = self.motion_bases_mlp(torch.ones_like(time_emb))  # using in global shared exp Shape: (batch_size, num_bases * 3)
        motion_bases = self.motion_bases_mlp(time_emb)
        motion_bases = motion_bases.view(-1, self.num_bases, 3) 
        
        hidden = self.query_time(rays_pts_emb, scales_emb, rotations_emb, time_emb).float()
        # dx = self.coeff_predictor(hidden)

        # Predict coefficients
        coeffs = self.coeff_predictor(hidden)  # Shape: (batch_size, num_bases)

        # Normalize coefficients to sum to 1
        coeffs = torch.softmax(coeffs, dim=-1)  # Shape: (batch_size, num_bases)

        # UPDATED: Compute position deformations using motion bases and coefficients
        # Compute position deformations
        dx = torch.sum(coeffs.unsqueeze(-1) * motion_bases, dim=1) 
        # import pdb; pdb.set_trace()
        ...
        dsh = self.sh_deform(hidden)

        return pts, scales, rotations, opacity, dsh
...
class deform_network(nn.Module):
    def __init__(self, args) :
        ...
        if self.use_res:
            ...
            self.deformation_net.coeff_predictor.initialize_weights()
    ...
    def forward_dynamic(self, point, scales=None, rotations=None, opacity=None, times_sel=None):
        ...
        means3D, scales, rotations, opacity, delta_sh = self.deformation_net( point,
                                                  scales,
                                                rotations,
                                                opacity,
                                                # times_feature,
                                                times_sel)
        return means3D, scales, rotations, opacity, delta_sh
    def get_mlp_parameters(self):
