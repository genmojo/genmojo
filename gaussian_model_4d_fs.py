...
from scene.deformation_sh import deform_network
...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, args, feat_dim: int = 20, feat_full_dim: int = 384):
        ...
        # NEW: feature dims + tensors
        self.feat_dim = feat_dim
        self.feat_full_dim = feat_full_dim
        self._feat = torch.empty(0)                      # [N, feat_dim]
        self.decoder = TinyDecoder(self.feat_dim, self.feat_full_dim).cuda()
    ...
    def freeze(self):
        ...
        # NEW
        if isinstance(self._feat, torch.Tensor) and self._feat.numel() > 0:
            self._feat.requires_grad_(False)
    ...
    def unfreeze(self):
        ...
        # NEW
        if isinstance(self._feat, torch.Tensor) and self._feat.numel() > 0:
            self._feat.requires_grad_(True)
        for p in self.decoder.parameters():
    ...
    def get_opacity(self):
        ...
        if o.dim() == 1:            # make it [N,1] consistently
            o = o.unsqueeze(1)
        return o
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        self._feat = nn.Parameter(
            torch.zeros((self.get_xyz.shape[0], self.feat_dim), device="cuda", dtype=torch.float32).requires_grad_(True)
        )
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
            {'params': [self._feat],                                    'lr': getattr(training_args, 'feature_lr', 1e-5),               "name": "feat"},
            {'params': self.decoder.parameters(),                        'lr': getattr(training_args, 'feature_lr', 1e-5),               "name": "feat_dec", 'weight_decay': 0.0},
        ]
    ...
    def construct_list_of_attributes(self):
        ...
        # NEW: 30-D compact features
        for i in range(self._feat.shape[1] if isinstance(self._feat, torch.Tensor) and self._feat.numel()>0 else self.feat_dim):
            l.append(f'dino_f_{i}')
        l.append('opacity')
    ...
    def save_ply(self, path):
        ...
        # NEW:
        feat_small = (self._feat.detach().contiguous().cpu().numpy()
                    if isinstance(self._feat, torch.Tensor) and self._feat.numel()>0
                    else np.zeros((xyz.shape[0], self.feat_dim), dtype=np.float32))
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, feat_small, opacities, scale, rotation), axis=1)  # NEW includes features
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)

        # NEW: save decoder sidecar
    ...
    def load_ply(self, path):
        ...
        # NEW: read 30-D features if present
        ...
        if len(dino_names) > 0:
            feat_small = np.stack([np.asarray(plydata.elements[0][n]) for n in dino_names], axis=1).astype(np.float32)
            self.feat_dim = feat_small.shape[1]
        else:
            feat_small = np.zeros((xyz.shape[0], self.feat_dim), dtype=np.float32)
        ...
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        self._feat = nn.Parameter(torch.tensor(feat_small, dtype=torch.float, device="cuda").requires_grad_(True))
        self._deformation = self._deformation.to("cuda")
        self._deformation_table = torch.gt(torch.ones((self.get_xyz.shape[0]),device="cuda"),0)

        # NEW: try to load decoder sidecar (falls back to fresh decoder)
        import glob, os
        stem = os.path.splitext(path)[0]
        cands = glob.glob(stem + "_featdec_*to*.pt")
        if len(cands) > 0:
            ckpt = torch.load(cands[0], map_location='cuda')
            self.feat_dim = int(ckpt.get('in_dim', self.feat_dim))
            self.feat_full_dim = int(ckpt.get('out_dim', self.feat_full_dim))
            self.decoder = TinyDecoder(self.feat_dim, self.feat_full_dim).cuda()
            self.decoder.load_state_dict(ckpt['state_dict'])
        else:
            self.decoder = TinyDecoder(self.feat_dim, self.feat_full_dim).cuda()
    ...
    def prune_points(self, mask):
        ...
        self._feat = optimizable_tensors["feat"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, new_feat):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_feat = self._feat[selected_pts_mask].repeat(N,1)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest,
                                new_opacity, new_scaling, new_rotation,
                                new_deformation_table, new_feat)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_feat          = self._feat[selected_pts_mask]
    ...
    def decode_feat_map(self, extra_cHW: torch.Tensor) -> torch.Tensor:
        """
        extra_cHW: [feat_dim, H, W] splatted by rasterizer
        returns:   [feat_full_dim, H, W]
        """
        C, H, W = extra_cHW.shape
        x = extra_cHW.permute(1,2,0).reshape(-1, C)   # [HW, feat_dim]
        y = self.decoder(x)                           # [HW, feat_full_dim]
        return y.view(H, W, self.feat_full_dim).permute(2,0,1).contiguous()